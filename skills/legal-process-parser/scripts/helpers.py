...
def build_pages(
    ...
    for number, raw in enumerate(texts, start=1):
        ...
        visual_outcome = processing["vision"].get("outcome")
        confirmed_nontext = processing["vision"].get("semantic_checked") and visual_outcome in {"blank", "image_only"}
        if confirmed_nontext and not text.strip() and processing["status"] == "COMPLETE":
            quality = "nontext_confirmed"
            warnings = [warning for warning in warnings if warning != "página sem texto extraído; requer visão/OCR"]
        page = Page(
            pdf_page=number,
            text=text,
            source="ocr" if number in ocr_page_set else (
                "native_text" if text.strip() else ("vision_review" if confirmed_nontext else "needs_ocr_or_vision")
            ),
            quality=quality,
...
def _build_structured_markdown_stage2(pages: Sequence[Page], classification: dict[str, Any]) -> str:
    ...
    for page in pages:
        entities = page.entities or {}
        missing_entity = "não identificado nesta página"
        entity_lines = [
            f"- {key}: {', '.join(values) if values else missing_entity}"
...
def build_page_processing_state(
    ...
    confirmed_nontext = description.get("outcome") in {"blank", "image_only"} and bool(description.get("semantic_description"))
    ...
    vision_state = {
        ...
        "attempted": bool(vision_enabled and trusted_vision_source),
    ...
    text_ok = bool(text.strip()) or ocr_used or bool(confirmed_nontext and semantic_checked)
    render_ok = (not render_required) or bool(render_state.get("validated"))
    vision_ok = (not vision_required) or semantic_checked
    if text_ok and render_ok and vision_ok and not explicit_unreadable:
        status = "COMPLETE_WITH_LIMITATION" if vision_state["limitations"] and semantic_checked else "COMPLETE"
    else:
        status = "PARTIAL"
    consolidation = {
        "completed": status == "COMPLETE",
        "layers": ["native_text", "render", "vision", "ocr", "metadata"],
    }
...
def _compute_coverage_stage3(pages: Sequence[Page], total: int) -> dict[str, Any]:
    ...
    unreadable = sum((page.vision or {}).get("outcome") == "unreadable" for page in pages)
    confirmed_blank = sum((page.vision or {}).get("outcome") == "blank" and page.status == "COMPLETE" for page in pages)
    image_only = sum((page.vision or {}).get("outcome") == "image_only" and page.status == "COMPLETE" for page in pages)
    unknown_content = sum(not page.text.strip() and page.status != "COMPLETE" and (page.vision or {}).get("outcome") != "unreadable" for page in pages)
    visual_records = [visual for page in pages for visual in (page.visuals or [])]
    ...
    textual_complete = bool(pages) and all(
        bool(page.text.strip()) or bool(page.ocr and page.ocr.get("used"))
        or (page.status == "COMPLETE" and (page.vision or {}).get("outcome") in {"blank", "image_only"})
        for page in pages
    )
    ...
    return {
        ...
        "pages_partial": partial,
        "pages_unreadable": unreadable,
        "pages_confirmed_blank": confirmed_blank,
        "pages_image_only": image_only,
        "pages_unknown_content": unknown_content,
        "failed_pages": failed,
        "pages_pending": list(range(processed + 1, total + 1)),
        "coverage_percent": round(100 * processed / total, 2) if total else 0,
...
def build_problematic_pages_report(pages: Sequence[Page]) -> str:
    ...
    for page in problematic:
        ...
        lines.append(
            ...
            f"{'OK' if render.get('validated') else 'PENDENTE'} | {'OK' if vision.get('semantic_checked') else 'PENDENTE'} | {safe_warning} |"
        )
    if not problematic:
