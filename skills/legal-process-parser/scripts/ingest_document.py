...
def _remove_stale_generated(output: Path, stale_names: set[str],
                            previous: dict[str, Any] | None,
                            protected: list[Path]) -> None:
    """Remove only files whose bytes still match the previous generated manifest."""
    if not previous:
        return
    previous_hashes = previous.get("artifact_hashes") or {}
    if not isinstance(previous_hashes, dict):
        return
    protected_resolved = {path.resolve() for path in protected}
    for relative_name, expected_hash in previous_hashes.items():
        if not isinstance(relative_name, str) or not isinstance(expected_hash, str):
            continue
        if not any(relative_name == name.rstrip("/") or relative_name.startswith(name.rstrip("/") + "/") for name in stale_names):
            continue
        path = output / relative_name
        resolved = path.resolve()
        if not resolved.is_relative_to(output.resolve()) or resolved in protected_resolved or not path.is_file():
            continue
...
def run_ingest(
    ...
    original_legal_review = legal_review.resolve() if legal_review else None
    if legal_review:
    ...
    protected = [source, *([vision_sidecar] if vision_sidecar else []), *([original_legal_review] if original_legal_review else [])]
    _remove_stale_generated(output, all_artifacts - set(required), existing, protected)
    ...
    if source.suffix.lower() == ".pdf" and page_descriptions:
        extra_pages = sorted(set(page_descriptions) - set(range(1, len(texts) + 1)))
        if extra_pages:
            raise ValueError(f"Revisão visual contém páginas inexistentes neste PDF: {extra_pages}")
    if source.suffix.lower() == ".pdf" and vision_policy == "required":
        missing_pages = sorted(set(range(1, len(texts) + 1)) - set(page_descriptions))
        if missing_pages:
            raise ValueError(f"Visão semântica obrigatória sem descrição da página inteira: páginas {missing_pages}")
        without_provider = sorted(number for number, desc in page_descriptions.items()
                                  if desc.get("description_source") not in {"vision_model", "human_review", "external_review"})
        if without_provider:
            raise ValueError(f"Revisão visual sem description_source verificável: páginas {without_provider}")
    ...
    recovery = build_recovery_report(pages, digest, extraction_warnings, render_warnings)
    ...
    manifest: dict[str, Any] = {
        ...
        "coverage": coverage,
        "recovery": {"status": recovery["status"], "findings": len(recovery["findings"]),
                     "queue": len(recovery["queue"]), "path": "recovery_findings.json"},
    ...
    write_json(output / "recovery_findings.json", recovery, encoding=output_encoding)
    ...
    manifest["execution_metrics"] = {"elapsed_seconds": round(time.perf_counter() - started_at, 3)}
    write_json(output / "manifest.json", manifest, encoding=output_encoding)
