...
try:
    ...
    from .readiness import evaluate_readiness, normalize_findings
except ImportError:
    from case_memory import page_revision
    from readiness import evaluate_readiness, normalize_findings
...
def check_support(review: dict, memory: dict) -> list[str]:
    errors = []
    readiness = evaluate_readiness(review, memory, normalize_findings(review))
    blocked_issues = {item["id"] for item in readiness["issue_statuses"] if item["analysis_status"] == "undetermined"}
    ...
    for issue in review.get("issues", []):
        for requirement in issue.get("requirements", []):
            if requirement.get("assessment") in {"missing", "disputed"} and not requirement.get("treatment") and issue.get("id") not in blocked_issues:
